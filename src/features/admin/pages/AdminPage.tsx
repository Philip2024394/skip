...
const AdminPage = () => {
  ...
  const handleMarkPaymentPaid = async (paymentId: string) => {
    const { error } = await supabase.from("payments").update({ status: "paid" } as any).eq("id", paymentId);
    if (error) { rlsErr(error); return; }
                ...
                failedPayments.map((p, i) => (
                  ...
                  >
                    ...
                    <div className="flex gap-2 pt-1">
                      ...
                      {(p as any).stripe_session_id && (
                        <button
                          onClick={() => { navigator.clipboard.writeText((p as any).stripe_session_id); toast.success("Session ID copied"); }}
                          className="px-3 py-2 rounded-xl text-[11px] font-bold flex items-center gap-1.5"
                          style={{ background: "rgba(99,102,241,0.12)", border: "1px solid rgba(99,102,241,0.25)", color: "#A5B4FC" }}
                        >
                          <CreditCard className="w-3.5 h-3.5" /> Copy ID
                        </button>
                      )}
                      <button
                      ...
                      >
                        <TrendingUp className="w-3.5 h-3.5" /> Stripe
                      </button>
                    </div>
                  </motion.div>
                ))
